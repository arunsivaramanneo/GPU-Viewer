...
MemoryTitle = ["Memory Types","Heap Index","Device Local","Host Visible","Host Coherent","Host Cached","Lazily Allocated"]
HeapTitle = ["Memory Heaps","Device Size","HEAP DEVICE LOCAL"]
QueueTitle = ["Queue Family","Queue Count","timestampValidBits","GRAPHICS BIT","COMPUTE BIT","TRANSFER BIT","SPARSE BINDING BIT","minImageTransferGranularity.width","minImageTransferGranularity.height","minImageTransferGranularity.depth"]
	...
	def Queues(GPUname):
		...
		for i in range(count):
			if i % 2 == 0:
				...
				background_color = "#ddd"
			QueueTab_Store.append([i,qCount[i],qBits[i],GBit[i],CBit[i],TBit[i],SBit[i],width[i],height[i],depth[i],background_color,Gfg[i],Cfg[i],Tfg[i],Sfg[i]])
		...
		with open("VKDLayer.txt","r") as file1:
			...
			for line in file1:
				...
				if i % 2 == 0:
					...
					background_color = "#ddd"
				LayerTab_Store.append([line.strip('\n'),Vversion[i].strip('\n'),LVersion[i].strip('\n'),ECount[i].strip('\n'),background_color])
				i = i + 1
	def Surface(GPUname):
		...
		for GPU in range(len(list)):
			if GPUname == GPU:
				...
				os.system("cat vulkaninfo.txt | awk '/Presentable Surfaces.*/{flag=1;next}/Device Properties and Extensions.*/{flag=0}flag' | awk '/GPU id       : %d.*/{flag=1;next}/VkSurfaceCapabilities.*/{flag=0}flag' | awk '{gsub(/count =.*/,'True');print}' | grep -v type >> VKDsurface.txt"%(GPU))
